...
function createDatabase(sqlitePath) {
  ...
  `);
    return {
        ...
        getLastSyncedBlock() {
            const row = getSyncState.get("last_synced_block");
        ...
        getTransfersByAddress(address, limit, offset) {
            const items = queryTransfers.all({ address, limit, offset });
            const row = countTransfers.get(address, address);
            return {
                items,
                total: row.count,
            };
